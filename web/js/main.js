...
function renderFileList() {
    filePreview.innerHTML = uploadedFiles.map((f, i) => `
        ...
        <button class="remove-btn" onclick="removeFile(${i})"><i class="fas fa-times"></i></button></div>
    `).join('');
}
...
function getCoords(e) {
    ...
    return { x: Math.min(canvas.width, Math.max(0, (cx - rect.left) * scaleX)), y: Math.min(canvas.height, Math.max(0, (cy - rect.top) * scaleY)) };
}
