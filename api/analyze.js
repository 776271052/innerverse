export default async function handler(req, res) {
  ...
  try {
    const { type, images, selfDesc, scope } = await req.json();
    ...
    if (!apiKey) {
      return res.status(500).json({ success: false, error: '未配置 SILICONFLOW_API_KEY' });
    }
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ 
        success: false, 
        error: `SiliconFlow 返回错误 (${response.status}): ${err}` 
      });
    }
...
export const config = {
  runtime: 'edge',
  regions: ['hnd1', 'iad1', 'pdx1', 'sfo1'] // 指定支持Edge Runtime的区域
};
