...
export async function analyzeWithAI(type, imageUrls, selfDesc = '') {
    ...
    const model = await getConfig(`model_${provider}_${modelKey}`);

    if (!model) {
        throw new Error(`未配置模型：model_${provider}_${modelKey}`);
    }
    ...
    if (provider === 'siliconflow') {
        ...
        if (!apiKey) throw new Error('未配置 SiliconFlow API Key，请在后台设置');
        return await callSiliconFlow(apiKey, model, messages);
    } else if (provider === 'deepseek') {
        const apiKey = await getConfig('deepseek_api_key');
        if (!apiKey) throw new Error('未配置 DeepSeek API Key，请在后台设置');
        ...
        if (!accountId || !token) throw new Error('未配置 Cloudflare AI 凭证，请在后台设置');
