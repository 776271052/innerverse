...
class FileListManager {
    ...
    async getBase64Array() {
        return Promise.all(this.files.map(f => fileToBase64(f)));
    }
}
...
const momentManager = new FileListManager({
    ...
    maxFiles: CONFIG.MAX_MOMENT_FILES
});
...
const chatManager = new FileListManager({
    ...
    maxFiles: CONFIG.MAX_CHAT_FILES
});
...
async function pollTaskStatus(taskId) {
    try {
        ...
        if (data.status === 'completed') {
            ...
            stopProgressUpdates();
            showMessage('error', '分析失败: ' + (data.error || '未知错误'));
            analyzing = false;
            stopPolling();
        } else {
            // 继续轮询，指数退避
        ...
        stopProgressUpdates();
        showMessage('error', '查询状态失败，请刷新重试');
        analyzing = false;
        stopPolling();
    }
}
...
analyzeMomentBtn.addEventListener('click', async () => {
    const images = await momentManager.getBase64Array();
    submitTask('moment', { images });
});

analyzeChatBtn.addEventListener('click', async () => {
    const images = await chatManager.getBase64Array();
    submitTask('chat', { images });
});
...
analyzeHtpBtn.addEventListener('click', () => {
    const dataURL = canvas.toDataURL('image/png');
    submitTask('htp', { image: dataURL });
});
...
async function loadAnnouncement() {
    try {
        ...
        if (data.text && data.text !== '') {
            announcementText.innerHTML = data.text;
            const closed = localStorage.getItem('announcementClosed');
            if (!closed || closed !== data.text) {
